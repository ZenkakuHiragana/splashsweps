...
---The power2_n table from Source SDK which is 2**(index - 128) / 255,
---but the former half and the latter half is swapped
---because string.byte returns an unsigned byte.
local power2 = {
...
---Conversion from linear light value to screenspace value
---directly taken from Source SDK to match the engine behavior.
---@type number[]
local linearToScreen = {}
for i = 1, 1024 do
    linearToScreen[i] = Clamp(floor(255 * pow((i - 1) / 1023, 1 / 2.2)), 0, 255)
end
...
function ss.BuildLightmapInfo(bsp, ishdr, surfaceInfo, cache)
    ...
    for i, rawFace in ipairs(faces) do
        ...
        if hasLightmap then
            if ((lightStyles[1] ~= 0 and lightStyles[1] ~= 255) or lightStyles[2] ~= 255) then
                t.HasLightmap = 2
            else
                t.HasLightmap = 1
            end
        end
        t.Width = t.HasLightmap and width or nil
        t.Height = t.HasLightmap and height or nil
        ...
        if maxLightmapIndex >= 2 then
            ...
            for j = maxLightmapIndex, 2, -1 do
                ...
                for k = 0, offset - 1 do
                    ...
                    -- e = bxor(e, 128) -- We don't need this anymore because the table is swapped already
                    v:SetUnpacked( -- TexLightToLinear
                        255 * r * power2[e + 1],
                        255 * g * power2[e + 1],
                        255 * b * power2[e + 1])
                    -- This uses Length() in the C++ but it shouldn't matter when comparing them.
                    local length = v:LengthSqr()
