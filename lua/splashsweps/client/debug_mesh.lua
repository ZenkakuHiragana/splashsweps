...
local function DrawHUDOverlays()
    ...
    if layers.info then
        ...
        local triCounts = { BASE = 0, CEIL = 0, SIDE = 0, DEPTH = 0, ["?"] = 0 }
        for _, data in ipairs(selectedSurfaces) do
            local vertices = data.vertices
        ...
        InfoLine(string.format("  BASE: %d  CEIL: %d  SIDE: %d  DEPTH: %d",
            triCounts.BASE, triCounts.CEIL, triCounts.SIDE, triCounts.DEPTH))
